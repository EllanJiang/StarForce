...
goto :comment
set index=0
for %%i in (xmlsForLiteNet\*.xml) do (
	set /a index=index+1
	echo 第!index!个文件为%%i
    rem xml路径，保存路径
	bin\proto_xml\ProtoForXml.exe  %curDir%%%i %rootDir%Assets\GameMain\Scripts\Protos\XmlProtosForLiteNet
)

:comment
bin\proto_xmlForLiteNet\ProtoForXml.exe  %curDir%xmlsForLiteNet %rootDir%Assets\GameMain\Scripts\Protos\XmlProtosForLiteNet
echo 协议转换完成
