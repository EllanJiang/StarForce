...
namespace LogicShared.LiteNetLib.Utils
{
    ...
    public class NetSerializer
    {
        ...
        private abstract class FastCall<T>
        {
            ...
            public virtual void Init(MethodInfo getMethod, MethodInfo setMethod, CallType callType) { Type = callType; }
        ...
        private abstract class FastCallSpecific<TClass, TProperty> : FastCall<TClass>
        {
            ...
            {
                ushort count = r.GetUShort();           //先读取数组长度
                var arr = GetterArr(inf);      //使用委托创建指定类型的数组
                arr = arr == null || arr.Length != count ? new TProperty[count] : arr;
                SetterArr(inf, arr);                    //使用委托设置数据
                return arr;
            }
            ...
            public override void Init(MethodInfo getMethod, MethodInfo setMethod, CallType callType)
            {
                base.Init(getMethod, setMethod, callType);
                switch (callType)
                {
                    case CallType.Array:
        ...
        {
            public static ClassInfo<T> Instance;            //类单例
            private readonly FastCall<T>[] _serializers;    //成员序列化器
            private readonly int _membersCount;             //类成员数量
